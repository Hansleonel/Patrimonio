...
public class SolicitudResource {
     ...
     *         solicitud, or with status 400 (Bad Request) if the solicitud has
     *         already an ID
     ...
     *         solicitud, or with status 400 (Bad Request) if the solicitud is not
     *         valid, or with status 500 (Internal Server Error) if the solicitud
     *         couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of solicitud in
     *         body
     */
    ...
    public ResponseEntity<List<Solicitud>> getAllSolicitud(Pageable pageable) throws InterruptedException {
        ...
        if (authentication.getAuthorities().toString().equals("[ROLE_USER]")) {
            page = solicitudService.getSolicitudByUser(pageable,
                    userRepository.findOneByLogin(authentication.getName()).get().getDociden());
        } else {
    ...
    @GetMapping("/solicitud-desplazamiento/entrantes")
    public ResponseEntity<List<Solicitud>> getAllSolicitudDesplazamientoEntrante(Pageable pageable)
            throws InterruptedException {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();

        List<Solicitud> solicitudes = solicitudService.getSolicitudDesplazamientoEntrantes(
                userRepository.findOneByLogin(authentication.getName()).get().getDociden());

        return new ResponseEntity<>(solicitudes, HttpStatus.OK);
    }

    @GetMapping("/solicitud-desplazamiento/salientes")
    public ResponseEntity<List<Solicitud>> getAllSolicitudDesplazamientoSaliente(Pageable pageable)
            throws InterruptedException {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();

        List<Solicitud> solicitudes = solicitudService.getSolicitudDesplazamientoSalientes(
                userRepository.findOneByLogin(authentication.getName()).get().getDociden());
    ...
    public ResponseEntity<Solicitud> patchPendienteRevisado(@PathVariable Long id, @RequestBody Solicitud solicitud)
            throws URISyntaxException {
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the solicitud,
